...
vector_t    *fredholm( unsigned degree, double lower, double upper, double (*g)( double ), double (*K)( double, double ), double lambda, unsigned fredholmType ) {
    ...
    b       = ChebyshevCoeff( degree + 1, lower, upper, g );                // g(x) vector
    ...
    matrix_print( stdout, AMatrixTitle, A, NULL );
...
extern  long double cheb_eval( double, double *, int, Function_Symmetry_t, double, double );
...
// ex131 - Atkinson, Problem 8 (14)

double  ex131_f( double x           ) {
    double  s   = sin( x );
    
    return 25.0 - 16.0 * s * s;
}

double  ex131_g( double x           ) { return 17.0 * 0.5 + ( 128.0 / 17.0 ) * cos( 2.0 * x );   }
double  ex131_K( double x, double y ) {
    double  c   = cos( 0.5 * ( x + y ) );
    
    return 0.3 / ( 1.0 - 0.64 * c * c );
}

// ex132 - Atkinson, Problem 8 (22)

double  ex132_f( double x           ) { return x;                                           }
double  ex132_g( double x           ) { return -( 2.0 * x * x * x - 9.0 * x + 2.0 ) / 6.0;  }
double  ex132_K( double x, double y ) { return fabs( x - y );                               }

// ex133 - Atkinson, Problem 8 (23),(24)

double  ex133_f( double x           ) { return sin( _pi * x );                                  }
double  ex133_g( double x           ) { return ( 1.0 - 1.0 / ( _pi * _pi ) ) * sin( _pi * x );  }
double  ex133_K( double x, double y ) { return ( x <= y )? x * ( 1.0 - y ) : y * ( 1.0 - x );   }

// ex134 - Atkinson, Problem 8 (42)

double  ex134_f( double x           ) { return 1.0; }
double  ex134_g( double x           ) { return 1.0; }
double  ex134_K( double x, double y ) { return 1.0; }

// ex135 - Atkinson, Problem 8 (43)

double  ex135_f( double x           ) { return 1.0; }
double  ex135_g( double x           ) { return 1.0; }
double  ex135_K( double x, double y ) { return 1.0; }

// ex136 - Atkinson, Problem 8 (50)

double  ex136_f( double x           ) { return 1.0; }
double  ex136_g( double x           ) { return 1.0; }
double  ex136_K( double x, double y ) { return 1.0; }

// ex137 - Atkinson, Problem 8 (51)

double  ex137_f( double x           ) { return 1.0; }
double  ex137_g( double x           ) { return 1.0; }
double  ex137_K( double x, double y ) { return 1.0; }

// ex138 - Atkinson, Problem 8 (57)

double  ex138_f( double x           ) { return 1.0; }
double  ex138_g( double x           ) { return 1.0; }
double  ex138_K( double x, double y ) { return 1.0; }

// ex139 - Atkinson, Problem 8 (58)
double  ex139_f( double x           ) { return 1.0; }
double  ex139_g( double x           ) { return 1.0; }
double  ex139_K( double x, double y ) { return 1.0; }


// examples - define the test cases as array of IntegralEquation_t

IntegralEquation_t  examples[] = {
    { "T0 - f(x)=K(x,y)=1, g(x)=3, [-1,+1]",                            2,  -1.0,   1.0,         1.0,        ex00_f,    ex00_K,   ex00_g,    2,  0.500,        0,  0  },
    { "T1 - f(x)=cos(x), K(x,y)=cos(x)cos(y), g(x)=B cos(x), [0,pi]",   2,   0.0,   _pi,         1.0,        ex01_f,    ex01_K,   ex01_g,    5,  0.125 * _pi,  0,  1  },
    { "T2 - f(x)=B exp(x), K(x,y)=g(x)=exp(x), [-1,+1]",                2,  -1.0,   1.0,         1.0,        ex04_f,    ex04_K,   ex04_g,    4,  0.500,        0,  1  },
    { "ex03 - Fox & Parker, Chapter 6 (6)",                             2,  -1.0,   1.0,        -1.0,        ex03_f,    ex03_K,   ex03_g,    3,  0.500,        0,  0  },

    { "ex61 - Fox and Goodwin, 4. Example 1 (13)",                      2,   0.0,   _pi_div_2,  -_4_div_pi,  ex61_f,    ex61_K,   ex61_g,    4,  0.125 * _pi,  0,  1  },
    ...
    { "ex63 - Fox and Goodwin, 11. Fredholm's Equation... (49)",        1,   0.0,   1.0,         1.0,        ex63_f,    ex63_K,   ex63_g,    1,  0.250,        0,  0  },
    
    { "ex12 - Numerical Solution of Integral Equations, pg 98",         2,   0.0,   _pi_div_2,  -0.25,       ex12_f,    ex12_K,   ex12_g,    4,  0.125 * _pi,  0,  1  },
    { "ex13 - Numerical Solution of Integral Equations, pg 104",        1,   0.0,   1.0,         1.0,        ex13_f,    ex13_K,   ex13_g,    5,  0.125,        0,  1  },
    { "ex14 - Numerical Solution of Integral Equations, pg 102",        2,   0.0,   1.0,         1.0,        ex14_f,    ex14_K,   ex14_g,    5,  0.125,        0,  1  },
    
    { "ex71 - R. E. Scraton, Example 1, (10) lambda = 2",               2,  -1.0,   1.0,        -2.0,        ex71_f1,   ex71_K,   ex71_g,    4,  0.250,        0,  1  },
    { "ex71 - R. E. Scraton, Example 1. (10) lambda = 1/2",             2,  -1.0,   1.0,        -0.5,        ex71_f2,   ex71_K,   ex71_g,    4,  0.250,        0,  1  },
    { "ex72 - R. E. Scraton, Example 1. (13)",                          2,  -1.0,   1.0,         0.5,        ex72_f,    ex72_K,   ex72_g,    6,  0.250,        0,  1  },
    
    { "ex81 - Baker et al, Example 2 (39)",                             1,   0.0,   1.0,         1.0,        ex81_f,    ex81_K,   ex81_g,    4,  0.125,        0,  1  },
    { "ex91 - Elliott, Two Examples (54)",                              2,  -1.0,   1.0,        -1.0,        ex91_f,    ex91_K,   ex91_g,   13,  0.200,        0,  1  },
    { "ex111 - Pilkington, Nystrom's method for solving... (II)",       2,  -1.0,   1.0,         1.0 / _pi,  ex111_f,   ex111_K,  ex111_g,   8,  0.250,        0,  1  },
    { "ex121 - Richardson, Introductory Example",                       2,   0.0,   1.0,        -0.5,        ex121_f,   ex121_K,  ex121_g,   5,  0.125,        0,  1  },
    
    { "ex101 - Rahbar and Hashemizadeh, Examples (13)",                 2,  -1.0,   1.0,        -1.0,        ex101_f,   ex101_K,  ex101_g,   2,  0.250,        0,  0  },
    { "ex102 - Rahbar and Hashemizadeh, Examples (14)",                 2,   0.0,   1.0,        -1.0,        ex102_f,   ex102_K,  ex102_g,   5,  0.125,        0,  1  },
    { "ex103 - Rahbar and Hashemizadeh, Examples (16)",                 2,   0.0,   1.0,         1.0,        ex103_f,   ex103_K,  ex103_g,  12,  0.125,        0,  1  },
    { "ex104 - Rahbar and Hashemizadeh, Examples (17)",                 2,  -1.0,   1.0,        -0.5,        ex104_f,   ex104_K,  ex104_g,  12,  0.250,        0,  1  },

    // XXX - unverified, untested

    { "ex131 - Atkinson, Problem  8 (14)",                              2,  -_pi,   _pi,         1.0 / _pi,  ex131_f,   ex131_K,  ex131_g,  16,  0.125 * _pi,  0,  1  },
    { "ex132 - Atkinson, Problem  4 (22)",                              2,   0.0,   1.0,        -1.0,        ex132_f,   ex132_K,  ex132_g,  10,  0.125,  0,  1  },
    { "ex133 - Atkinson, Problem  5 (23),(24)",                         2,   0.0,   1.0,        -1.0,        ex133_f,   ex133_K,  ex133_g,  10,  0.125,  0,  1  },
    { "ex134 - Atkinson, Problem  4 (42)",                              2,   0.0,   2.0,        -1.0,        ex134_f,   ex134_K,  ex134_g,   6,  0.250,  0,  0  },
    { "ex135 - Atkinson, Problem  5 (43)",                              2,   0.0,   2.0,        -1.0,        ex135_f,   ex135_K,  ex135_g,   6,  0.250,  0,  0  },
    { "ex136 - Atkinson, Problem  3 (50)",                              2,   0.0,   1.0,        -1.0,        ex136_f,   ex136_K,  ex136_g,   6,  0.125,  0,  0  },
    { "ex137 - Atkinson, Problem  4 (51)",                              2,   0.0,   _pi_div_2,  -1.0,        ex137_f,   ex137_K,  ex137_g,   6,  0.125,  0,  0  },
    { "ex138 - Atkinson, Problem  1 (57)",                              2,   0.0,   1.0,        -1.0,        ex138_f,   ex138_K,  ex138_g,   6,  0.125,  0,  0  },
    { "ex139 - Atkinson, Problem 10 (58)",                              2,   0.0,   1.0,        -1.0,        ex139_f,   ex139_K,  ex139_g,   6,  0.125,  0,  0  },
};
