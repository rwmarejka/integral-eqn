/*
% cc -O -Xc -o int-mt int-mt.c -lm -lthread
 *
...
static char	*SCCSid	= "@(#) integral.c 2.3 94/02/08 Richard Marejka";
...
MatrixOp( matrix_t *a, void *(*op)( void * ), void *opd ) {
	...
	for ( i=0; i < nCPU; ++i ) {
		int	n;
#define	THR_FLAGS	THR_BOUND
		if ( n = thr_create( NULL, 0, op, &arg[i], THR_FLAGS, &tid[i] ) ) {
			for ( ; i; i-- )
